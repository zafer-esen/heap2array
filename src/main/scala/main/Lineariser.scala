...
import ap.types.UninterpretedSortTheory.UninterpretedSort
import ap.{Signature, _}
...
import ap.parser.IExpression.{Predicate, Quantifier, Sort}
import ap.types._
import ap.util.{Debug, Seqs}

import scala.collection.mutable.{ArrayBuffer, LinkedHashMap, HashMap => MHashMap, HashSet => MHashSet, Map => CMap}

/**
 * This file is a modification of Princess SMTLineariser.scala.
 */
...
object Lineariser {
  ...
  private def findTheorys(formulas : Iterable[IFormula],
    ...
    def checkSort(s : Sort) : Unit = s match {
      ...
      case s : UninterpretedSort => theoryCollector addTheory s.theory
      case s => // nothing
    }
  ...
  def apply(formulas : Seq[IFormula],
    ...
    val lineariser = new Lineariser(benchmarkName,
      logic, status,
 ...
 */
class Lineariser(benchmarkName : String,
                 logic : String,
                 status : String,
                 constsToDeclare : Seq[ConstantTerm],
                 predsToDeclare : Seq[Predicate],
  ...
  import Lineariser.{quoteIdentifier, toSMTExpr, escapeString,
    trueConstant, falseConstant, eqPredicate,
    printSMTType, printADTDeclarations, sort2SMTString}
  ...
  def open : (CMap[IFunction, IFunction], CMap[Predicate, Predicate]) = {
    ...
    // declare address sorts
    for (heap <- heaps) {
      println("(define-sort " + heap.AddressSort.name + " () Int)")
    }

    // declare other sorts
    for (theory <- theoriesToDeclare) {
      theory match {
        case _ : ADT  => // nothing
        case _ : Heap => // nothing
    ...
    val adts = for (theory <- theoriesToDeclare;
                    if (theory match {
                      case adt : ADT =>
                        !heaps.exists(h => adt == h.AllocResADT)
                      case _ : Heap => false // not printing Heap declarations
                      case _ : UninterpretedSortTheory => false
                      case _ => false
                    }))
      yield theory.asInstanceOf[ADT]
    printADTDeclarations(adts)

    // declare Heap sort, AllocRes sort and heap operations
    for (heap <- heaps) {
      val addrName = heap.AddressSort.name
      val heapName = heap.HeapSort.name
      val objName = heap.ObjectSort.name
      ...
      println("(declare-datatypes ((AllocRes" + heapName + " 0) (" + heapName + " 0))\n" +
        ...
        "       h))\n" +
        "(define-fun alloc" + heapName + "   ((h " + heapName + ") (o " + objName + ")) AllocRes" + heapName + "\n" +
    ...
    val heapFunMap = new MHashMap[IFunction, IFunction]
